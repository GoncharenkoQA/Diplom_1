...
class TestBurger:
    @allure.title('Проверка метода set_buns, проверка добавления булки в бургер')
    def test_set_buns_success(self, mock_bun):
        burger = Burger()
        burger.set_buns(mock_bun)
        assert burger.bun == mock_bun

    @allure.title('Проверка метода add_ingredient, проверка добавления ингредиентов в бургер')
    @allure.description('С помощью параметризации выполняются 3 теста: проверка по очереди '
                        'добавления соуса и 2 разных начинок')
    ...
    @allure.title('Проверка метода remove_ingredient, проверка удаления ингредиента из бургера')
    @allure.description('С помощью параметризации выполняется 2 теста: проверка удаления '
                        ' соуса и проверка удаления начинки')
    ...
    @allure.title('Проверка метода move_ingredient, проверка перемещения ингредиентов в бургере')
    ...
    @allure.title('Проверка метода get_price, проверка получения конечной стоимости бургера')
    ...
    @allure.title('Проверка метода get_receipt, проверка получения стоимости бургера И его рецепта')
